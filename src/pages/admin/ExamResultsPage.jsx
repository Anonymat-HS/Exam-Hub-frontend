...
export function ExamResultsPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      examService.getExamDetail(selectedExamId),
      resultService.getExamResults(selectedExamId),
    ]).then(([detailRes, resultsRes]) => {
      if (detailRes.status === 'fulfilled') {
        setExamDetail(detailRes.value);
      } else {
        const msg = detailRes.reason instanceof ApiError ? detailRes.reason.message : 'Erreur de chargement des détails.';
        setError(msg);
      }
      if (resultsRes.status === 'fulfilled') {
        setResultsData(resultsRes.value);
      } else {
        const msg = resultsRes.reason instanceof ApiError ? resultsRes.reason.message : 'Erreur de chargement des résultats.';
        setError(msg);
      }
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-6">
          <ErrorMessage message={error} onRetry={() => setSelectedExamId((id) => id)} />
        </div>
      )}
          ...
          {isLoadingResults ? (
            ...
            <>
              ...
              <div className="mt-6 overflow-hidden rounded-2xl bg-white shadow-sm">
                <ResultTable results={results} scoreMax={scoreMax} examTitle={selectedExam?.title ?? ''} />
              </div>
            </>
          )}
        </>
